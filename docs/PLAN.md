...
**Фича** — небольшой проект со своим манифестом и набором файлов-узлов (обычно фича соответствует экрану).

**Узел (файл в фиче):**
- **ID** — UUID, производный от пути к файлу в проекте.
- **Имя файла** и **хеш файла** (content-hash).
- Поля `type`, `props`, `children` не используются — вся иерархия задаётся манифестом.

**Манифест:**
- Описывает дерево файлов и папок проекта (фичи) с дополнительной информацией (id, имя, хеш для каждого узла).
- Формат: XML, JSON, YAML или другой, подходящий для задачи.
- Есть **корневая фича**, в которой прописаны остальные фичи и их расположение в файлах.
- Манифест на всё приложение для данной платформы включает версию приложения на этой платформе; минимальная единица — файл экрана или элемент экрана.

**Дельта:**
- Строится **сервером** при сравнении версии клиента и версии, накатанной через админку.
- Хранится **только в кешах сервера**; клиент получает уже готовую дельту по запросу (по фиче/экрану).

**Потоки:**
1. **Холодный старт:** клиент без кеша запрашивает фичу и версию → сервер отдаёт полный набор файлов фичи и манифест; клиент кеширует и рендерит.
2. **Проверка обновлений:** клиент передаёт версию платформы и идентификатор фичи; сервер достаёт соответствующий манифест, сравнивает с опубликованной версией, строит дельту (или достаёт из кеша), отдаёт дельту клиенту; клиент применяет дельту.
...
- [plan/ios.md](plan/ios.md) — iOS (SwiftUI SPM nahui3)
- [plan/android.md](plan/android.md) — Android (Kotlin Compose nahui3)
- [plan/web.md](plan/web.md) — Web (React nahui3)
...
| Delta и согласованность клиент/сервер | Есть версия платформы на клиенте и опубликованная версия этой же платформы на сервере. Клиент отправляет версию платформы и фичу, которую смотрит. Сервер достаёт соответствующий манифест, сравнивает с опубликованным манифестом, строит дельту (или достаёт из кеша), сохраняет её в кеш при необходимости и отправляет дельту клиенту; клиент применяет её. |
...
| **ios** | SwiftUI SPM nahui3: загрузка фичи по манифесту и файлам, передача версии платформы, применение дельты, рендер. |
| **android** | Kotlin Compose nahui3: загрузка фичи, передача версии, применение дельты, рендер. |
| **web** | React nahui3: загрузка фичи, передача версии, применение дельты, рендер. |
